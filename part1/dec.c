...
const int MEMORY_TO_ACC = ACC_TO_MEMORY; // Both are the same code but they
                                         // differ in a flag before the w
...
#define EXPR(name, reg, s)                                                     \
    RegisterExpression name = {.registers = reg, .size = s}

#define EXPR_SINGLE(name, reg)                                                 \
    const RegisterExpression name = {.registers = reg, .size = 1}
...
RegisterExpression arr[][2] = {
    {AL_EXP, AX_EXP}, {CL_EXP, CX_EXP}, {DL_EXP, DX_EXP}, {BL_EXP, BX_EXP},
    {AH_EXP, SP_EXP}, {CH_EXP, BP_EXP}, {DH_EXP, SI_EXP}, {BH_EXP, DI_EXP},
};
...
const RegisterExpression arr1[] = {
    {.registers = BX_SI, 2}, {.registers = BX_DI, 2}, {.registers = BP_SI, 2},
    {.registers = BP_DI, 2}, {.registers = SI, 1},    {.registers = DI, 1},
    {.registers = BP, 1},    {.registers = BX, 1},
};
...
Operation *double_raw_operation(char *opName, int idx, int size,
                                RegisterExpression leftReg,
                                RegisterExpression rightReg) {
...
DecodedOperations *decode(Memory *mem, int size) {
    ...
    for (int idx = 0; idx < size; idx += bytesRead) {
        ...
        if (opName == IMMEDIATE_TO_REG) {
            ...
            operation = multi_reference_operation(
                MOV, idx, sizeof(ImdToRegOperation) + (regOp->w ? 2 : 1));
            ...
            operation->right->value =
                regOp->w ? read2Bytes(mem->data + bytesRead + idx)
                         : readByte(mem->data + bytesRead + idx);
            operation->wide = regOp->w;
            bytesRead += regOp->w ? 2 : 1;
        } else if (opName == MEM_REG_TO_MEM_REG || opName == ADD_REG ||
                   opName == SUB_REG || opName == CMP_REG) {
            ...
            case 1: {
                ...
                char *opName = op->op_code == 11   ? SUB
                               : op->op_code == 15 ? CMP
                                                   : ADD;
                bytesRead += sizeof(ImdToAccAddOperation);
                operation = multi_reference_operation(
                    opName, idx,
                    sizeof(ImdToRegOperation) + (op->w == 1 ? 2 : 1));
            ...
            case 32: {
                ImdToRegMemAddOperation *op =
                    malloc(sizeof(ImdToRegMemAddOperation));
                memcpy(op, mem->data + idx, sizeof(ImdToRegMemAddOperation));
                bytesRead += sizeof(ImdToRegMemAddOperation);

                Displacement *disp = read_displacement_ptr(
                    op->mod, op->rm, mem->data + idx + bytesRead);
                bytesRead += disp->size;
                ...
                char *opName = op->math_code == 7   ? CMP
                               : op->math_code == 5 ? SUB
                                                    : ADD;
                operation = multi_reference_operation(opName, idx, bytesRead);
                operation->right->type = Value;
                ...
                if (isRawReg) {
                    ...
                    operation->left->addr =
                        isDirectAccess ? EMPTY : arr1[op->rm];
                    ;
                    operation->left->value = disp->value;
                    operation->left->type =
                        isDirectAccess ? DirectAccess : Expression;
                    operation->left->disp = disp;
                }
            } break;
            case 34:
            ...
            case 14: {
                ...
                char *opName = op->op_code == 34   ? MOV
                               : op->op_code == 10 ? SUB
                               : op->op_code == 14 ? CMP
                                                   : ADD;
                if (op->mod == 3) {
                    RegisterExpression dest =
                        arr[op->d == 1 ? op->reg : op->rm][op->w];
                    RegisterExpression src =
                        arr[op->d == 1 ? op->rm : op->reg][op->w];
                    operation = double_raw_operation(
                        opName, idx, sizeof(RegToRegOperation), dest, src);
                    operation->wide = op->w;
                } else {
                    Displacement *disp = read_displacement_ptr(
                        op->mod, op->rm, mem->data + idx + bytesRead);
                    ...
                    operation = multi_reference_operation(
                        opName, idx, sizeof(RegToRegOperation) + disp->size);
                    ...
                    if (op->mod == 1) {
                        ...
                        RegisterExpression dest =
                            arr[(op->d == 0 || (op->mod == 0)) ? op->reg
                                                               : op->rm][op->w];
                        operation->left->addr = dest;
                        operation->left->type = Raw;
                        operation->wide = op->w;
                        // Everywhere should look if it's wide
                        if (op->rm == 6 && op->mod == 0) {
            ...
            operation = multi_reference_operation(MOV, idx,
                                                  sizeof(ImdToRegMemOperation) +
                                                      disp->size +
                                                      (op->w == 0 ? 1 : 2));
            operation->right->type = Value;
            operation->right->prefix = prefix;
            operation->right->value = data;
            operation->wide = op->w == 1;
            if (op->mod == 3) {
                operation->left->type = Raw;
                operation->left->addr = arr[op->rm][op->w];
            } else {
            ...
            operation = jmp_operation(
                opName, ipinc, idx, sizeof(JMPOp) + (jumpOp->size == 1 ? 1 : 2),
                jmp->label);
            ...
            operation = jmp_operation(jmpName, jumpOp->value, idx,
                                      sizeof(CondJMPOP), jmp->label);
        }
